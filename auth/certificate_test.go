...
-----END CERTIFICATE-----`
	...
	err3 := ValidateClientCertificate(ctx, crt, "127.0.0.1:8080", true)
	...
	err4 := ValidateClientCertificate(ctx, crt, "127.0.0.1:8080", true)
	...
	err5 := ValidateClientCertificate(ctx, crt, "127.0.0.1:8080", false)
	suite.Nil(err5)
}
...
func (suite *CertificateTestSuite) TestExtractIPSANs() {
	...
	crt.Extensions = []pkix.Extension{
		{
			...
			Value: []byte{0x04, 0x04, 0x7F, 0x00, 0x00, 0x01}, // IP address 127.0.0.1 in DER format
		},
	}
	suite.True(HasIPSANs(crt))

	crt.Extensions = []pkix.Extension{}
	suite.False(HasIPSANs(crt))
