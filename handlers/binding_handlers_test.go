...
func (suite *BindingHandlersSuite) TestBindingUpdate() {
 ...
 "created_on": "2018-05-05T15:04:05Z"
	...
	req, err := http.NewRequest("POST", "http://localhost:8080/service-types/s1/hosts/host1/bindings/test_dn_1", bytes.NewBuffer([]byte(postJSON)))
	if err != nil {
		log.Error(err.Error())
	}
...
func (suite *BindingHandlersSuite) TestBindingUpdateUnknownServiceTypeInPATH() {
 ...
 "error": {
  ...
  "status": "NOT FOUND"
	...
	req, err := http.NewRequest("GET", "http://localhost:8080/service-types/unknown/hosts/host1/bindings/test_dn_1", nil)
	if err != nil {
		log.Error(err.Error())
	}
